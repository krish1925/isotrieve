/**
 * Isotrieve Performance Benchmarks
 * Compares Isotrieve vs text serialization
 */

import { Isotrieve } from '../packages/core/src/protocol';
...
async function benchmarkCalibration(vocabSize: number): Promise<BenchmarkResult> {
  const agent1 = new Isotrieve({ embedder: new BenchmarkEmbedder(384, 'model-1') });
  const agent2 = new Isotrieve({ embedder: new BenchmarkEmbedder(768, 'model-2') });
...
async function benchmarkSimilaritySearch(kbSize: number, queries: number): Promise<BenchmarkResult> {
  const agent = new Isotrieve({ embedder: new BenchmarkEmbedder(384, 'model') });
...
async function runBenchmarks() {
  console.log(' Isotrieve Performance Benchmarks\n');
