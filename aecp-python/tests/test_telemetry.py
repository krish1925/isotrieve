...
from aecp.wrappers.telemetry import log_query, read_telemetry, summary


@pytest.fixture(autouse=True)
def _clean_telemetry(tmp_path, monkeypatch):
    """Ensure telemetry is off and path points to tmp."""
    monkeypatch.setenv("AECP_TELEMETRY", "off")
    monkeypatch.setattr(
        "aecp.wrappers.telemetry._telemetry_path",
...
class TestTelemetry:
    ...
    def test_local_writes(self, tmp_path, monkeypatch):
        monkeypatch.setenv("AECP_TELEMETRY", "local")
