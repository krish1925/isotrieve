...
class TestPackageImport:
    ...
    def test_import_gate(self):
        from aecp.quality.gate import GateVerdict
    ...
    def test_import_migrate(self):
        from aecp.migrate import migrate_store
...
def _aecp_bin() -> str | None:
    """Find the aecp CLI script next to the current Python interpreter."""
    aecp = Path(sys.executable).parent / "aecp"
    if aecp.exists():
        return str(aecp)
    return shutil.which("aecp")


def _run_aecp(*args: str) -> subprocess.CompletedProcess[str]:
    bin_path = _aecp_bin()
    if bin_path is None:
        pytest.skip("aecp CLI not installed")
...
class TestCLI:
    ...
    def test_cli_help(self):
        ...
        assert result.returncode == 0
        assert "aecp" in result.stdout.lower()
