...
mapped through the fitted AECP transform.  Batching is preserved;
...
class AECPOpenAI:
    ...
    Output vectors are mapped through the AECP transform before return.
    ...
    def __init__(
        ...
        *,
        ...
        self._embeddings = _AECPEmbeddingsNamespace(self)

    @property
    def embeddings(self) -> _AECPEmbeddingsNamespace:
        return self._embeddings

    def _map_vectors(self, vecs: np.ndarray) -> np.ndarray:
        """Map vectors through the AECP transform."""
    ...
    def __repr__(self) -> str:
        return (
            f"AECPOpenAI(mapping_type={self._mapping.mapping_type!r}, "
            f"d_src={self._d_src}, d_tgt={self._d_tgt})"
        )


class _AECPEmbeddingsNamespace:
    """Namespace matching ``client.embeddings.create(...)``."""

    def __init__(self, shim: AECPOpenAI) -> None:
        self._shim = shim
    ...
    def create(
        ...
        **kwargs: Any,
        ...
        """Create embeddings and map through AECP transform.
