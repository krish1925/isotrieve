...
from isotrieve.providers.cached import CachedEmbedder, with_disk_cache
...
def _create_raw(model_id: str) -> Embedder:
    ...
    if mid.startswith("openai:"):
        from isotrieve.providers.openai import OpenAIEmbedder

        return OpenAIEmbedder(mid.split(":", 1)[1])
    if mid.startswith("voyage:"):
        from isotrieve.providers.voyage import VoyageEmbedder

        return VoyageEmbedder(mid.split(":", 1)[1])
    if mid.startswith("cohere:"):
        from isotrieve.providers.cohere import CohereEmbedder

        return CohereEmbedder(mid.split(":", 1)[1])
    if mid.startswith("gemini:"):
        from isotrieve.providers.gemini import GeminiEmbedder
    ...
    from isotrieve.providers.sentence_transformers import SentenceTransformerEmbedder
