...
Env ``ISOTRIEVE_TELEMETRY=local`` enables writing to ``~/.cache/isotrieve/telemetry.jsonl``.
"""
...
def _telemetry_enabled() -> bool:
    ...
    return os.environ.get("ISOTRIEVE_TELEMETRY", "off").lower() == "local"


def _telemetry_path() -> Path:
    """Return the local telemetry JSONL path."""
    return Path.home() / ".cache" / "isotrieve" / "telemetry.jsonl"
