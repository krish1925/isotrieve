"""Frozen generic calibration corpus (isotrieve-calib-v1).
...
def load_calib_v1() -> list[str]:
    """Return the frozen isotrieve-calib-v1 text list."""
    # Prefer packaged JSON if present (future expansion); else embedded tuple.
    try:
        root = resources.files("isotrieve.calibration")
        data_path = root.joinpath("data", "isotrieve_calib_v1.json")
