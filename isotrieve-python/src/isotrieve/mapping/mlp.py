...
from isotrieve.mapping.base import Mapping, _check_finite, l2_normalize
from isotrieve.mapping.linear import _holdout_metrics
...
class ResidualMLPMapping(Mapping):
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray) -> ResidualMLPMapping:
        try:
            ...
            raise ImportError(
                "ResidualMLPMapping requires torch. Install with: pip install isotrieve[mlp]"
            )
