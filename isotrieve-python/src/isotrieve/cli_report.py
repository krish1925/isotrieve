...
def register_report_command(app: typer.Typer) -> None:
    """Register the ``isotrieve report`` command on the Typer app."""

    @app.command("report")
    def report_cmd() -> None:
        """Summarize wrapper usage and print graduation commands.

        Reads local telemetry (ISOTRIEVE_TELEMETRY=local) and prints
        the exact ``isotrieve gate`` / ``isotrieve migrate`` commands to
        graduate from query-time wrapper to full migration.
        """
        from isotrieve.wrappers.telemetry import summary
        ...
        if s["total_queries"] == 0:
            console.print(
                ...
                "Enable with: export ISOTRIEVE_TELEMETRY=local\n"
