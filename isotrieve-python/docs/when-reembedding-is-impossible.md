...
corpus is not possible. Isotrieve is the only option in these scenarios.
...
**Isotrieve's approach:** The gate measures retention using *queries*, which usually still
...
**Isotrieve's approach:** Use `isotrieve calibrate --queries-only` with a query log. Queries
...
**Isotrieve's approach:** Same as compliance deletion — query-only calibration. If you
...
**Isotrieve's approach:** Standard calibration with paired texts. The preprocessing
...
exist even when documents don't. This is the fundamental reason Isotrieve works in these
...
Even in these scenarios, Isotrieve may not be the right choice:
...
- If the calibration sample is too small (< 2000 texts), the mapping quality
  degrades. Use `isotrieve gate` to verify.
