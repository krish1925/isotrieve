...
({{ reembed_calls }} embedding calls × ${{ cost_per_call }}). Isotrieve fits a mapping from
...
> model pair, run `isotrieve gate` on your corpus — never guess.

## Step-by-step

```bash
# 1. Estimate cost
isotrieve plan --source-model {{ SOURCE_MODEL }} \
          --target-model {{ TARGET_MODEL }} \
          --corpus-size {{ corpus_size }}

# 2. Calibrate (use in-domain texts from your corpus)
isotrieve calibrate --source-model {{ SOURCE_MODEL }} \
               --target-model {{ TARGET_MODEL }} \
...
# 4. If PASS, migrate
isotrieve transform --mapping mapping.isotrieve \
...
- **Dimension change** ({{ source_dim }} → {{ target_dim }}): Isotrieve handles
