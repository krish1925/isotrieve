...
class TestIsotrieveChromaFunction:
    """Test IsotrieveChromaFunction (serve-mode EmbeddingFunction)."""
    ...
    def test_raises_without_embedder(self):
        ...
        ef = IsotrieveChromaFunction(m, new_model_embedder=None)
    ...
    def test_embed_query(self):
        ...
        ef = IsotrieveChromaFunction(m, new_model_embedder=embed_fn)
...
class TestMigrateCollection:
    ...
    def test_basic_migration(self):
        ...
        assert (
            "isotrieve_mapping_id" in report.metadatas
        ...
        # Verify Isotrieve metadata was added
        for meta in target_data["metadatas"]:
            assert "isotrieve_mapping_id" in meta
            assert meta["isotrieve_format_version"] == 1
            assert meta["isotrieve_source_collection"] == "test_col"
    ...
    def test_double_migration_warning(self):
        ...
        src_col = _FakeChromaCollection(
            embeddings=rng.normal(size=(10, 8)).tolist(),
            ids=[str(i) for i in range(10)],
            metadatas=[{"isotrieve_mapping_id": "already_migrated"} for _ in range(10)],
            documents=[None] * 10,
        )
